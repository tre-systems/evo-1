...
```javascript
import init, { createBattleSimulation, ParallelProcessor } from "./pkg/battleo.js";

await init();

const simulation = await createBattleSimulation("canvas");
...
- `createBattleSimulation(canvas_id?: string) -> Promise<BattleSimulation>`
...
Use `createBattleSimulation` for the browser UI because WebGPU device setup is asynchronous. The synchronous constructor is retained as a compatibility fallback path and initializes the WebGL/Canvas2D renderer.
